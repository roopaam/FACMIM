...
class MRMRSelector:
    """
    Minimum Redundancy Maximum Relevance selector.

    Objective:

        score_j = I(X_j ; Y) - beta * redundancy_j

    where:

        redundancy_j = mean_{X_s in selected} I(X_j ; X_s)

    For the first selected feature, redundancy_j = 0.
    """
    ...
    def fit(
        ...
        for rank in range(1, n_to_select + 1):
            ...
            for c in remaining:
                ...
                score = float(marginal_relevance[c] - self.redundancy_weight * redundancy)
    ...
    def get_support(self) -> list[str]:
        if not self.is_fitted_:
            raise RuntimeError("MRMRSelector must be fitted before get_support().")
    ...
    def get_support_mask(self) -> np.ndarray:
        if not self.is_fitted_:
            raise RuntimeError("MRMRSelector must be fitted before get_support_mask().")
    ...
    def get_diagnostics(self) -> pd.DataFrame:
        if not self.is_fitted_:
            raise RuntimeError("MRMRSelector must be fitted before get_diagnostics().")

        return self.diagnostics_.copy()

    def get_feature_scores(self) -> pd.DataFrame:
        if not self.is_fitted_:
            raise RuntimeError("MRMRSelector must be fitted before get_feature_scores().")

        return self.feature_scores_.copy()


# Backward-compatible aliases.
mRMRSelector = MRMRSelector
MRMRBaselineSelector = MRMRSelector
