...
def run_compas_smoke(output_dir: Path):
    cmd = [
        sys.executable,
        "scripts/run_compas_pilot.py",
        "--synthetic_smoke",
        "--sample_size",
        "300",
        "--k",
    ...
    return pd.read_csv(output_dir / "compas_pilot_results.csv")


def test_compas_pilot_synthetic_smoke_runs(tmp_path):
    output_dir = tmp_path / "compas_smoke"

    df = run_compas_smoke(output_dir)
...
def test_compas_pilot_contains_all_selector_families(tmp_path):
    output_dir = tmp_path / "compas_smoke"
...
def test_compas_selected_features_do_not_include_sensitive_column(tmp_path):
    output_dir = tmp_path / "compas_smoke"
